...
class CommentController extends BaseController
{
	...
	public function publish()
	{
		...
		if ($user->authorise('core.edit', 'com_blogg') || $user->authorise('core.edit.state', 'com_blogg'))
		{
			$model = $this->getModel('Comment', 'Site');
			...
			if (!$item)
			{
				...
				$this->setRedirect(Route::_('index.php?option=com_blogg&view=comments', false));
			}
			else
			{
	...
	public function checkin()
	{
		...
		if ($user->authorise('core.manage', 'com_blogg') || $item->checked_out == $user->id) { 
			...
			if ($return === false)
			{
				...
				$this->setRedirect(Route::_('index.php?option=com_blogg&view=comment' . '&id=' . $id, false), $message, 'error');
				return false;
			}
			else
			{
				// Checkin succeeded.
				$message = Text::_('COM_BLOGG_CHECKEDIN_SUCCESSFULLY');
				$this->setRedirect(Route::_('index.php?option=com_blogg&view=comment' . '&id=' . $id, false), $message);
				return true;
			}
		}
		else
	...
	public function remove()
	{
		...
		if ($user->authorise('core.delete', 'com_blogg'))
		{
			$model = $this->getModel('Comment', 'Site');
			...
			else
			{
				...
				$this->app->redirect(Route::_('index.php?option=com_blogg&view=comments', false));
			}
